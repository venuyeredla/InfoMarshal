...
/**
 * From mattmahony PFAQ.
 * @author vyeredla
 *
 */
public class ABSCompressor {
...
class Predictor {
	private int cxt; // Context: last 0-8 bits with a leading 1
	private int[] t=new int[512]; // Probability of 1
...
class Encoder {
	...
	private Mode mode; // Compress or decompress?
	private ByteBuf buffer; // Compressed data file
    private int x1, x2;   // Range, initially [0, 1), scaled by 2^32
    private int x;        // Last 4 input bytes of archive.
    public Encoder(Mode mode, ByteBuf buffer) {
    	this.buffer=buffer;
    	this.mode=mode;
    	this.x1=0;
    	this.x2=0xffff;
    	...
    	if (mode==Mode.DECOMPRESS) {
    	    for (int i=0; i<4; ++i) {
					int c = buffer.readByte(); //int c=getc(archive);
	    		     x=(x<<8)+c;
    		  }
    	}
      }
    ...
    public void encode(int y) {  // Compress bit y
    	  // Update the range
    	  int xmid = x1 + ((x2-x1) >> 12) * predictor.p();
    	  ...
    	  while (((x1^x2)&0xff000000)==0) {
    		  buffer.writeByte(x2>>24);
    	      x1<<=8;
    	      x2=(x2<<8)+255;
    	  }
    }
   public  int decode() {  // Uncompress and return bit y
	  // Update the range
	  int xmid = x1 + ((x2-x1) >> 12) * predictor.p();
	   assert(xmid >= x1 && xmid < x2);
	   int y=0;
	 ...
	 public  void flush() {  // Call when done compressing
		 if (mode==Mode.COMPRESS)
			  buffer.writeByte(x2>>24);// First unequal byte
	     }
...
class ABSEncoder {
	private Predictor predictor;
	private Mode mode;  // Compress or decompress?
    private final int N=10;      			// Number of bits in state x, max 12
    private final int R=7;       // Number of bits in r (quantized q), max 7
    private final int B=500000; // Input stack size (compression) 
    private final int BO=B*2+128;// Output stack size in bits, a multiple of 8, max
                        // 0xfffff8 (compression).  It should reserve enough
                        // space > B in case the compressor expands the input.
    ByteBuf archive;    // Compressed data file
    private int x;            // Encoder state (decompression)
    private int n;            // Number of bits in input stack (compression)
                    // or remaining in block (decompression)
    private int qr;     // q -> r quantization
    //private short (*enc)[1<<N]; // rd,w -> k,x (compression)
    //short (*dec)[1<<N];    // r,x -> d,x>>k (decompression)
     int[] ins;    // Input stack (compression)
     int[] outs;   // Output stack (compression)
     int ch;                // Input byte (decompression)

     public ABSEncoder(Mode m, ByteBuf buffer) {
